...
import {
    ...
    FileText, User, Calendar, Package, Box, Image as ImageIcon, CheckSquare, Check // ✅ เพิ่ม Check
} from "lucide-react";
...
interface PickingSlip {
    ...
    status: 'pending' | 'in_progress' | 'completed' | 'cancelled' | 'assigned' | 'done';
    created_at: string;
    warehouse_id: string;
    picker_name?: string;
    order?: {
...
export default function PickingShow({ auth, pickingSlip }: Props) {
    ...
    const canStartPicking = ['pending', 'assigned', 'in_progress'].includes(pickingSlip.status);
            ...
            <Head>
                ...
                <style>{`
                    @media print {
                        ...
                        /* ซ่อนรูปในโหมด Print เพื่อประหยัดหมึก (Optional) */
                        ...
                        <div className="flex items-center gap-2 w-full sm:w-auto">
                            ...
                            {canStartPicking && (
                                <Button onClick={handleStartPicking} className="flex-1 sm:flex-none gap-2 bg-indigo-600 hover:bg-indigo-700 shadow-md shadow-indigo-200 text-white">
                                    ...
                                    {pickingSlip.status === 'pending' ? 'Accept & Start' : 'Continue Picking'}
                                </Button>
                            )}
                        </div>
                    </div>
                                ...
                                <table className="w-full text-sm">
                                    <thead className="bg-gray-50 text-gray-500 font-semibold uppercase text-xs">
                                        <tr>
                                            ...
                                            <th className="py-3 text-center w-24">Loc.</th>
                                                ...
                                                <tr key={index} className="group page-break hover:bg-gray-50/50 transition-colors">
                                                    ...
                                                    <td className="py-4 align-top">
                                                        <div className="flex gap-4">
                                                            {/* รูปภาพ (ซ่อนตอน Print ถ้าต้องการ) */}
                                                            {item.image_url ? (
                                                    ...
                                                    {/* Location */}
                                                    <td className="py-4 align-top text-center font-mono text-gray-400">
                                                        {item.location || '-'}
                                                    </td>
                                                    ...
                                                    <td className="py-4 align-top text-center border-l border-gray-100">
                                                        {isPicked ? (
                                                            // ถ้าหยิบครบแล้ว -> แสดงเครื่องหมายถูก (สีเขียวบนจอ, สีดำตอน Print)
